  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-slate-700 bg-mentisCard">
      <div className="border-b border-slate-700 px-4 py-3">
        <h2 className="text-sm font-semibold text-white">AI Career Assistant</h2>
          ...
          <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
              msg.role === "user"
                ...
                : "border border-slate-700 bg-mentisBg/70 text-white"
            }`}>
              {msg.content}
            </div>
          </div>
      ...
      <div className="border-t border-slate-700 p-3">
        <div className="flex gap-2">
          <textarea
            ...
            className="flex-1 resize-none rounded-xl border border-slate-700 bg-mentisBg px-3 py-2 text-sm text-white placeholder-slate-500 focus:border-mentisPrimary focus:outline-none"
          />
          <button
            onClick={sendMessage}
